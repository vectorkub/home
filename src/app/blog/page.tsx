            ...
            {categories.map((category) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-medium ${category === "All"
                    ? "bg-white text-black"
                    : "border border-white/15 text-white/60 transition hover:border-white/40 hover:text-white"
                  }`}
              >
                {category}
              </button>
              ...
              <Link href={`/blog/${post.slug}`}>
                ...
                >
                  <div className="flex items-center gap-2 text-xs text-white/60">
                    <span className="rounded-full border border-white/15 px-2 py-1">{post.category}</span>
                    <span>{post.readTime}</span>
                  </div>
                  <h2 className="mt-3 line-clamp-2 text-lg font-semibold text-white">{post.title}</h2>
                  <p className="line-clamp-3 text-sm leading-relaxed text-white/60">{post.excerpt}</p>
          ...
          <div className="relative space-y-4">
            ...
            <div className="mx-auto flex w-full max-w-md flex-col gap-3 sm:flex-row">
              <input
                ...
                className="flex-1 rounded-full border border-white/15 bg-black/40 px-4 py-3 text-sm text-white placeholder:text-white/40 focus:border-white/40 focus:outline-none"
              />
